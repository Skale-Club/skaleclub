...
// every occurrence.
//
// Pricing facts in the PT copy match the EN source exactly: the volume tiers
// generated by buildPriceLines() (shared/nfc-price-lines.ts) from
// shared/nfc-pricing.ts, pedido mínimo de 20 peças (US$ 200 no preço da
// faixa de 20 peças), taxa de arte de US$ 50 só no primeiro pedido, pagamento
...
const PT_TRANSLATIONS: Array<{ source: string; translated: string }> = [
  ...
  // Shared verbatim by BOTH leadFormCta sections (landing and pricing) since
  // plan fix/nfc-campaign-readiness pointed both at the priced order form —
  // one row covers both CTAs.
  {
    source:
    ...
    translated:
      "Leva cerca de um minuto para preencher. O formulário mostra o preço conforme você escolhe a quantidade, você envia a sua logo, e confirmamos tudo com você no WhatsApp antes de produzir qualquer coisa. Enviar o formulário não cobra nada de você.",
  },
  ...
  {
    source: "To get started, we need your logo as a file (PNG, JPG, WEBP or PDF) and the link you want the tap to open. If you do not have a logo file, we create the artwork for you — that is what the first-order art fee covers.",
    translated: "Para começar, precisamos da sua logo em arquivo (PNG, JPG, WEBP ou PDF) e do link que o toque deve abrir. Se você não tem o arquivo da logo, a gente cria a arte para você — é isso que a taxa de arte do primeiro pedido cobre.",
  },

  // ── Pricing page: pricingTable ──────────────────────────────────────────
  // Plan fix/nfc-campaign-readiness switched the `lines` on this section from
  // hand-typed flat pricing to buildPriceLines() (shared/nfc-price-lines.ts,
  // the same generator scripts/seed-nfc-order-page.ts and PricingTableSection.tsx's
  // default lines use). The generated
  // strings ("20-49 pieces", "$10.00 each", "20 pieces", "$200.00", "$50.00", ...)
  // are ALREADY seeded by scripts/seed-nfc-order-translations.ts for the order
  // page — reuse them there rather than duplicating. "Per keychain" and
  // "20 pieces × $10" below are now stale (no page renders them), left in place
  // because nothing in this codebase deletes translation rows.
  ...
  { source: "Why is there a minimum order?", translated: "Por que existe um pedido mínimo?" },
  {
    ...
    translated: "Cada pedido é configurado, desenhado, impresso e programado em lote, então tiragens muito pequenas não fazem sentido para nenhum dos lados. O mínimo é de 20 peças, o que dá US$ 200 no preço da faixa de 20 peças — e o preço por peça cai conforme você pede mais. Vinte peças já são suficientes para colocar uma em cada ponto de contato e ainda distribuir para a sua equipe.",
  },
  ...
  // The answer to the question above no longer lives here: plan
  // fix/nfc-campaign-readiness rewrote it to reuse, verbatim, the "No logo
  // file? Upload the best version you have..." string already seeded by
  // scripts/seed-nfc-order-translations.ts (it matches how the priced order
  // form itself handles a missing logo). Do not add a duplicate pair here —
  // one row already covers both pages, per this file's own convention above.
  //
  // The row below (source starting "That is fine. Choose...") is now stale —
  // no page references that exact string anymore. Left in place: neither this
  // script nor scripts/lib/seed-translations.ts deletes translation rows.
  {
  ...
  // Same PT value as "See pricing and details" above — both read naturally as
  // "Ver preços e detalhes" in Brazilian Portuguese; keep them in agreement.
