...
You: "Excelente, João! Um especialista entrará em contato em até 24 horas para discutir como podemos ajudar seu negócio a crescer!"`;
  ...
  // Reorder portfolio services (admin only)
...
${untranslated.map((t, i) => `${i + 1}. ${t}`).join('\n')}`;
  ...
  // --- VCard API Routes ---
  app.get("/api/vcards", requireAdmin, async (req, res) => {
    try {
      const { vcards } = await import("#shared/schema.js");
  ...
  app.post("/api/vcards", requireAdmin, async (req, res) => {
    try {
      const { vcards, insertVCardSchema } = await import("#shared/schema.js");
