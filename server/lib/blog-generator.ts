...
import { BLOG_CONTENT_MODEL, BLOG_IMAGE_MODEL, BLOG_GEMINI_TIMEOUT_MS, getBlogGeminiClient, resolveBlogGeminiApiKey } from "./blog-gemini.js";
import { selectNextRssItem } from "./rssTopicSelector.js";
import { GeminiEmptyResponseError, GeminiTimeoutError, getPlainTextLength, sanitizeBlogHtml, slugifyTitle as slugifyTitleNFD } from "./blogContentValidator.js";
...
// (@google/genai 1.50.x has no native AbortSignal support).
async function withGeminiTimeout<T>(label: string, run: (signal: AbortSignal) => Promise<T>): Promise<T> {
...
function isTransientError(err: unknown): boolean {
  ...
  // 5xx = transient; 4xx (auth, quota, malformed) NOT retried — Gemini quota windows are minutes.
...
// Phase 38 BLOG2-16: composes over withGeminiTimeout. On transient error, sleeps
...
async function withGeminiRetry<T>(label: string, run: (signal: AbortSignal) => Promise<T>): Promise<T> {
  let lastErr: unknown;
...
function parseGeneratedPostResponse(raw: string): GeneratedPost {
  ...
  if (tags.length === 0) throw new Error("Gemini blog generation returned incomplete content: tags are required");
  return {
...
async function generateTopicWithGemini({ settings, manual, rssItem }: { settings: BlogSettings; manual: boolean; rssItem: BlogRssItem }): Promise<string> {
...
async function generatePostWithGemini({ settings, topic, manual, rssItem }: { settings: BlogSettings; topic: string; manual: boolean; rssItem: BlogRssItem }): Promise<GeneratedPost> {
  resolveBlogGeminiApiKey();
  const client = getBlogGeminiClient();
...
async function generateImageWithGemini({ post }: { settings: BlogSettings; post: GeneratedPost; manual: boolean }): Promise<Buffer | null> {
  ...
  const response = await withGeminiRetry<any>("image", () =>
    (client.models as any).generateContent({ model: BLOG_IMAGE_MODEL, contents: prompt }),
  );
...
async function uploadFeatureImage({ bytes, path }: { bytes: Buffer; path: string }): Promise<string> {
  ...
  const { error } = await supabase.storage.from("images").upload(path, bytes, { contentType: "image/jpeg", upsert: false });
...
async function getGeminiText(client: ReturnType<typeof getBlogGeminiClient>, model: string, prompt: string): Promise<string> {
  const response = await (client.models as any).generateContent({ model, contents: prompt });
  // D-08: distinct typed error when Gemini returns no candidates (content filter, model outage).
  if (!response?.candidates || response.candidates.length === 0) {
  ...
  const fallback = response?.candidates?.[0]?.content?.parts
    ...
    .join("\n")
    .trim();
  if (!fallback) throw new Error(`Gemini ${model} returned no text output`);
  return fallback;
}
...
export class BlogGenerator {
  static async generate({ manual, rssItemId }: { manual: boolean; rssItemId?: number }): Promise<BlogGeneratorResult> {
    ...
    try {
      ...
      if (error instanceof GeminiTimeoutError) reason = "gemini_timeout";
