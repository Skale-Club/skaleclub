...
export function IntegrationsSection() {
  ...
  const [googlePlacesSettings, setGooglePlacesSettings] = useState<{provider: string; apiKey: string; isEnabled: boolean; hasKey?: boolean}>({
  ...
  useEffect(() => {
    if (groqSettingsData) {
      ...
      if (groqSettingsData.hasKey) {
        ...
        setGroqApiKey((current) => current || MASKED_OPENAI_KEY);
      } else {
  ...
  const saveGroqSettings = async (enabled?: boolean) => {
    ...
    try {
      await apiRequest('PUT', '/api/integrations/groq', {
        ...
        apiKey: groqApiKey && groqApiKey !== MASKED_OPENAI_KEY ? groqApiKey : undefined,
      });
  ...
  const testGroqConnection = async () => {
    ...
    try {
      const response = await fetch('/api/integrations/groq/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ apiKey: groqApiKey && groqApiKey !== MASKED_OPENAI_KEY ? groqApiKey : undefined }),
        credentials: 'include',
      });
  ...
  const saveGooglePlacesSettings = async (settingsToSave?: { apiKey?: string; isEnabled?: boolean }) => {
    ...
    try {
      await apiRequest('PUT', '/api/integrations/google-places', {
        apiKey: settingsToSave?.apiKey !== undefined && settingsToSave.apiKey !== MASKED_API_KEY 
      ...
      toast({ 
        title: 'Failed to save Google Places settings', 
        description: error.message, 
        variant: 'destructive' 
      });
    } finally {
  ...
  const handleToggleGooglePlaces = async (checked: boolean) => {
    if (checked && googlePlacesTestResult !== 'success' && !googlePlacesSettings.hasKey) {
      toast({
        title: 'Please run Test Connection',
        description: 'You must have a successful test before enabling Google Places.',
        variant: 'destructive'
      });
      return;
    }
  ...
  const testGooglePlacesConnection = async () => {
    ...
    try {
      const response = await fetch('/api/integrations/google-places/test', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          apiKey: googlePlacesApiKey && googlePlacesApiKey !== MASKED_API_KEY ? googlePlacesApiKey : undefined
        }),
        credentials: 'include'
      });
      const result = await response.json();
      ...
      if (result.success) {
        ...
        toast({ 
          title: 'Connection failed', 
          description: result.message || 'Could not connect to Google Places',
          variant: 'destructive'
        });
      }
    } catch (error: any) {
    ...
    <div className="space-y-8">
      ...
      <Card className="border-0 bg-muted">
        ...
        <CardContent className="space-y-3">
          ...
          {groqTestMessage && (
            <div className={`flex items-center gap-2 rounded-md border px-2.5 py-2 text-xs ${groqTestResult === 'success' ? 'border-green-200 bg-green-50 text-green-700 dark:border-green-800 dark:bg-green-900/20 dark:text-green-400' : 'border-red-200 bg-red-50 text-red-700 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400'}`}>
          ...
          <div className="flex gap-2">
            <Button
              ...
              disabled={isTestingGroq || (!groqApiKey || groqApiKey === MASKED_OPENAI_KEY)}
            >
            ...
            <Button
              ...
              disabled={isSavingGroq || (!groqApiKey || groqApiKey === MASKED_OPENAI_KEY)}
            >
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-3">
              <Input
                ...
                onFocus={() => {
                  if (googlePlacesApiKey === MASKED_API_KEY) {
                    setGooglePlacesApiKey('');
                  }
                }}
                onBlur={() => {
                  if (!googlePlacesApiKey && googlePlacesSettings?.hasKey) {
                    setGooglePlacesApiKey(MASKED_API_KEY);
                  }
                }}
