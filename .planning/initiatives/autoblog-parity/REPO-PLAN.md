...
| **SC-12** | Tests: schedule, pillar fallback when RSS is empty, Telegram callback auth | — | **DONE** — 68 tests across `contract`, `schedule`, `editorial-pillars`, `telegram-targets`, `telegram-approvals` (callback parsing against hostile input, constant-time secret compare, the forum-topic strip on callback chat matching) and `token-crypto`. |
