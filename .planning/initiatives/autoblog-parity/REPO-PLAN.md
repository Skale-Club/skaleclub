...
| **SC-07** | Telegram groups | xkedule | **PARTLY DONE** — `chat_ids text[]` with backfill, multi-destination fan-out, forum-topic threads, validation at save time, admin UI takes a comma-separated list, and the test button reports per-chat failures. The approvals columns exist but the **approval webhook, callback handling and `setWebhook` reconcile are still to port** |
