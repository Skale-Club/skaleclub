...
- âœ… **v1.0 Xpot Tech Debt** â€” Phases 1-4 (shipped 2026-03-30)
- âœ… **v1.1 Multi-Forms Support** â€” Phase 5 / M3 (shipped 2026-04-15)
- âœ… **v1.2 Estimates System** â€” Phases 6-9 (shipped 2026-04-20)
- âœ… **v1.3 Links Page Upgrade** â€” Phases 10-14 (shipped 2026-04-20)
- âœ… **v1.4 Admin Presentations Page** â€” Phases 15-20 (shipped 2026-04-22)
- âœ… **v1.5 Blog Post Automation** â€” Phases 21-24 (shipped 2026-04-24)
...
## Shipped Milestones
<details>
<summary>âœ… v1.6 Skale Hub Weekly Live Gate (Phases 25-29) â€” SHIPPED 2026-05-02</summary>
...
- [x] Phase 5: Multi-Forms Support (1/1 plans) â€” completed 2026-04-15
  ...
  - M3-05: cleanup â€” drop legacy endpoints + form_config column
...
| 30. AI Provider + Transcription Settings | v1.7 | 1/1 | Complete | 2026-05-22 |
| 31. Form Engine Hardening | v1.7 | 1/1 | Complete | 2026-05-22 |
| 32. Public Voice Capture + Basic Lead Summary | v1.7 | 1/1 | Complete | 2026-05-22 |
| 33. Branch Builder UX | v1.7 | 0/1 | Not started | - |
| 34. Voice Review + Lead Brief UX | v1.7 | 0/1 | Not started | - |
| 35. UAT + Provider Verification | v1.7 | 0/1 | Not started | - |

---

_Last updated: 2026-05-22 — v1.7 active; Phases 30-32 complete; Phases 33-35 remaining_
...
**Success Criteria** (what must be TRUE):
  1. Admin can `POST /api/uploads/links-page` with an image (â‰¤2 MB) and receives a Supabase `uploads/links-page/...` URL that is publicly retrievable.
  2. `linksPageConfig` persists per-link `iconType`, `iconValue`, `visible`, `clickCount`, stable `id`, and a `theme` sub-object â€” existing `links` and `socialLinks` read/write paths still work unchanged.
...
**Plans:**
- [x] 10-01-PLAN.md â€” Extend linksPageConfig schema with real Zod + lazy UUID/theme normalization in storage read path (LINKS-01, LINKS-03)
- [x] 10-02-PLAN.md â€” POST /api/uploads/links-page admin endpoint with MIME allowlist, 2 MB cap, Supabase scoped path (LINKS-02, LINKS-06)
...
**Plans:**
- [x] 11-01-PLAN.md â€” Public IP-rate-limited POST /api/links-page/click/:linkId + admin click-count data surface (LINKS-04, LINKS-05)
...
**Success Criteria** (what must be TRUE):
  1. Admin sees a three-zone layout â€” Profile (left), Live Preview placeholder (center), Links + Social (right) â€” that stacks cleanly on narrow screens and uses the existing `AdminCard`/`SectionHeader`/`FormGrid` primitives.
  2. Admin can drag-and-drop an image onto the Avatar or Background uploader, sees a spinner, then a thumbnail + "Uploaded âœ“" confirmation, and the URL is persisted to `linksPageConfig`.
...
**Plans:**
3/3 plans executed
- [x] 12-02-PLAN.md â€” DragDropUploader for avatar/background (LINKS-08)
- [x] 12-03-PLAN.md â€” Drag-reorder links (LINKS-11)
**UI hint**: yes

### Phase 13: Icon Picker, Theme Editor & Live Preview
**Goal**: Admin can fully brand each link (icon + theme) and see their edits reflected inside the admin page within ~1 second â€” no round-trip to `/links` required.
**Depends on**: Phase 12 (needs the three-zone layout + link rows to host the picker)
**Requirements**: LINKS-09, LINKS-12, LINKS-13
**Success Criteria** (what must be TRUE):
  1. Each link row has an Icon Picker where the admin can either search the full Lucide icon library (debounced text search, ~1000 icons) and select one, or upload a custom SVG/PNG that becomes the link's icon â€” the selected icon renders at its final display size in the picker preview.
...
- [x] 13-01-PLAN.md â€” IconPicker component (Popover+Tabs, Lucide search, Upload, Auto) wired per link row (LINKS-09)
- [x] 13-02-PLAN.md â€” ThemeEditor for primary/background/gradient + Reset to defaults in Profile zone (LINKS-12)
- [x] 13-03-PLAN.md â€” LivePreview iframe in Zone 2 with auto-refresh on save + manual refresh (LINKS-13)
**UI hint**: yes

### Phase 14: Public Page Rendering + Click Tracking
**Goal**: Visitors to `/links` see a themed, icon-rich page that only shows visible links, and every outbound click increments the admin-visible counter reliably â€” even when the browser immediately navigates away.
...
**Success Criteria** (what must be TRUE):
  ...
  3. `/links` reflects the saved theme â€” primary color on hover/focus accents, background color or gradient on the page root, and optional background image rendered behind the ambient glow layer.
...
**Plans:**
- [x] 14-01-PLAN.md â€” Visible filter + per-link icons + theme application + sendBeacon click tracking in client/src/pages/Links.tsx (LINKS-14, LINKS-15, LINKS-16, LINKS-17)
**UI hint**: yes
...
**Plans:** 2/2 plans complete
**Success Criteria** (what must be TRUE):
  1. A raw SQL migration script runs without error and creates `presentations`, `presentation_views`, and `brand_guidelines` tables with the correct columns, constraints, and cascade-delete rules â€” `SELECT * FROM presentations LIMIT 1` returns an empty result set, not an error.
  ...
  3. `server/lib/anthropic.ts` exports `getAnthropicClient()` and calling it with `ANTHROPIC_API_KEY` set returns a live client that can reach the Anthropic API â€” it is not wired through the existing `getActiveAIClient()` OpenAI/Groq shim.
  4. `package.json` lists `@anthropic-ai/sdk` as a production dependency and `npm run check` passes cleanly.
**Plans:**
- [ ] 15-01-PLAN.md â€” SQL migration (3 tables) + Drizzle/Zod schema + barrel re-export + storage stubs (PRES-01, PRES-02, PRES-03)
- [ ] 15-02-PLAN.md â€” @anthropic-ai/sdk install + getAnthropicClient() singleton in server/lib/anthropic.ts (PRES-04)

### Phase 16: Admin CRUD API
**Goal**: Admins can create, list, update, and delete presentations through a typed REST API with no AI involvement â€” the data layer is fully operational before any UI or AI work begins.
**Depends on**: Phase 15 (tables and storage interface must exist)
**Requirements**: PRES-05, PRES-06, PRES-07, PRES-08
**Plans:** 1/1 plans complete
**Success Criteria** (what must be TRUE):
  1. `GET /api/presentations` (admin-auth required) returns a JSON array sorted by `createdAt` desc; each item includes `id`, `slug`, `title`, derived `slideCount`, derived `viewCount`, and `createdAt` â€” an unauthenticated request returns 401.
...
**Plans:**
- [x] 16-01-PLAN.md â€” registerPresentationsRoutes (GET list, POST create, PUT update, DELETE delete) + IStorage interface declarations (PRES-05, PRES-06, PRES-07, PRES-08)
...
**Plans:** 1 plan
**Success Criteria** (what must be TRUE):
  1. `PUT /api/brand-guidelines` (admin-auth required) upserts a single row; a subsequent `GET /api/brand-guidelines` (no auth required) returns the saved `content` string â€” calling `GET` before any `PUT` returns a 200 with empty or null content rather than a 404.
...
**Plans:**
- [ ] 17-01-PLAN.md â€” GET + PUT /api/brand-guidelines routes + PresentationsSection editor + AdminSection wiring (PRES-09, PRES-10)
**UI hint**: yes

### Phase 18: AI Authoring Endpoint
**Goal**: A single POST endpoint accepts a chat message and the current slide state, calls Claude via `tool_use` with brand guidelines as the system prompt, and streams structured `SlideBlock[]` JSON back to the client â€” the entire AI pipeline is exercisable before any admin UI is built.
...
**Plans:** 2/2 plans complete
**Success Criteria** (what must be TRUE):
  1. `POST /api/presentations/:id/chat` (admin-auth required) accepts `{ message: string }`, loads the current `brand_guidelines.content` as Claude's system prompt, and returns a streaming `text/event-stream` response â€” calling it with `curl --no-buffer` shows `data:` events arriving progressively before the stream closes.
  ...
  4. Sending "edit slide 3 â€” shorten the body" when there are 5 slides returns an updated array where only slide index 2 is modified and slides 0, 1, 3, 4 are byte-for-byte identical to the input; the DB write reflects this partial update.
**Plans:**
- [x] 18-01-PLAN.md â€” Wave 0: ANTHROPIC_API_KEY in .env.example + slideBlockSchema Zod unit test (PRES-12)
- [x] 18-02-PLAN.md â€” Wave 1: server/routes/presentationsChat.ts SSE endpoint + route registration (PRES-11, PRES-12, PRES-13)

### Phase 19: Admin Presentations Editor
**Goal**: Admin can manage presentations and edit their slides via a JSON editor with a live mini-preview â€” slides are authored by conversing with Claude Code (IDE) and pasting the resulting JSON.
**Depends on**: Phase 16 (CRUD API for list/create/delete)
**Requirements**: PRES-14, PRES-15, PRES-16
**Plans:** 1/1 plans complete
Plans:
- [ ] 19-01-PLAN.md â€” PresentationsSection list + JSON editor + slide mini-cards + Admin.tsx wire-up (PRES-14, PRES-15, PRES-16)
**Success Criteria** (what must be TRUE):
...
**Goal**: Anyone with a presentation link can experience the deck as a fullscreen bilingual scroll-snap presentation â€” isolated from the site's Navbar and Footer â€” with an access code gate if one is set, and every successful view is recorded for admin analytics.
...
Plans:
- [x] 20-01-PLAN.md â€” Server endpoints (slug augment + verify-code + view POST) + App.tsx isPresentationRoute guard + translations (PRES-17, PRES-18, PRES-22)
- [x] 20-02-PLAN.md â€” PresentationViewer.tsx â€” scroll-snap, 8 layout renderers, language switcher, access gate, view tracking (PRES-19, PRES-20, PRES-21)
**Success Criteria** (what must be TRUE):
  1. `GET /api/presentations/slug/:slug` (no auth) returns the full presentation including slides; each successful response creates a new row in `presentation_views` â€” the admin view-count badge for that presentation increments by 1 after reload.
  ...
  4. Appending `?lang=pt-BR` to the URL switches all slide text to the `headingPt`, `bodyPt`, and `bulletsPt` fields; appending `?lang=en` (or no param) shows the English fields â€” switching does not reload the page or restart the scroll position.
...
### Phase 25: Services Carousel Polish âœ… COMPLETE (2026-04-29)
...
**Requirements:** No formal REQ-IDs â€” polish phase added post-milestone
**Depends on:** Phase 24
**Plans:** 2 plans

Plans:
- [x] 25-01: ServicesCarousel â€” fade mask + click threshold + paused prop (commit `1c723f6`)
- [x] 25-02: ServiceDetailModal â†’ shadcn Dialog + carousel pause on open (commit `1423e5e`)

---

_Last updated: 2026-04-22 â€” Phase 20 complete (2/2 plans); PRES-17â€“22 all delivered; v1.4 feature-complete_
